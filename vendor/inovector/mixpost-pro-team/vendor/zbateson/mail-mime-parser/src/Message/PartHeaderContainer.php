...
class PartHeaderContainer implements IteratorAggregate
{
    ...
    private function getAllWithOriginalHeaderNameIfSet(string $name) : ?array
    {
        ...
        if (isset($this->headerMap[$s])) {
            $self = $this;
            $ed = \array_($this->headerMap[$s], function($h) use ($name, $self) {
                return (\strcasecmp($self->headers[$h][0], $name) === 0);
            });
            return (!empty($ed)) ? $ed : $this->headerMap[$s];
        }
        return null;
    }
    ...
    {
        return \array_(\array_map([$this, 'getByIndex'], \array_keys($this->headers)));
    }
    ...
    {
        return \array_values(\array_($this->headers));
    }
