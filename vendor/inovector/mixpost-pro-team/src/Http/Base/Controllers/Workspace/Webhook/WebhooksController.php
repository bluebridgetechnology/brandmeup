...
class WebhooksController extends Controller
{
    ...
    public function store(StoreWebhook $storeWebhook): RedirectResponse
    {
        ...
        return redirect()
            ->route('br.webhooks.edit', ['workspace' => WorkspaceManager::current()->uuid, 'webhook' => $record->uuid])
    ...
    public function destroy(Request $request): RedirectResponse
    {
        ...
        if (!$query) {
            return redirect()
                ->route('br.webhooks.index', ['workspace' => $request->route('workspace')])
                ->with('error', __('mixpost::webhook.not_found'));
        }

        return redirect()->route('br.webhooks.index', ['workspace' => $request->route('workspace')])
