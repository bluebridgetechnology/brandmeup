...
const schedule = (postNow = false) => {
    ...
    axios.post(route('br.posts.schedule', { workspace: workspaceCtx.id, post: postId.value, isWorkspaceAdminRole: isWorkspaceAdminRole.value }), {
        postNow
    }).then((response) => {
        notify('success', response.data.message, {
            name: $t("post.view_in_calendar"),
            href: route('br.calendar', { workspace: workspaceCtx.id, date: props.form.date })
        });
