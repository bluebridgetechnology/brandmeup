...
import AttachUserInvite from "./AttachUserInvite.vue";
...
const detachUser = (user) => {
    confirmation()
        ...
        .onConfirm((dialog) => {
            ...
            router.delete(route('br.workspaces.users.delete', {workspace: props.workspace.uuid}), {
                    data: {
                        user_id: user.id
                    },
                    preserveScroll: true,
...
const updateRole = () => {
    formRole.put(route('br.workspaces.users.update', {workspace: props.workspace.uuid}), {
            preserveScroll: true,
            onSuccess() {
                formRole.reset();
                closeModalRole();
...
<template>
    <Panel>
        ...
        <template #action>
            <AttachUserInvite :workspace="workspace" :attachedUsers="workspace.users.map((item) => item.id)"/>
        </template>
