...
const openWorkspace = (workspace) => {
    ...
    if (usePage().component === 'Main/ErrorPage') {
        router.post(route('br.switchWorkspace', {workspace: workspace.uuid, redirect: true}));
        return;
    }

    router.post(route('br.switchWorkspace', {workspace: workspace.uuid}), {}, {
        onSuccess() {
            window.location.replace(route('br.dashboard', {workspace: workspace.uuid}));
        },
        onError() {
            notify('error', 'Error!');
        }
...
<template>
    <Dropdown width-classes="w-64" placement="top-start">
        ...
        <template #content>
            ...
            <DropdownItem v-if="user.is_admin" :href="route('br.dashboardAdmin')">
                <template #icon>
                    <CogIcon/>
                </template>
            ...
            <DropdownItem :href="route('br.profile.index')">
                <template #icon>
                    <PencilSquareIcon/>
                </template>
            ...
            <template v-if="features.api_access_tokens">
                <DropdownItem :href="route('br.profile.accessTokens.index')">
                    <template #icon>
                        <KeyIcon/>
                    </template>
            ...
            <DropdownItem :href="!impersonating ? route('br.logout') : enterpriseConsole.stop_impersonating_url"
                          linkAs="button"
