...
const {addValidationError, removeValidationError} = usePostValidator();
...
const validateContentDisclosure = (account) => {
    if (getContentDisclosure(account) && (!getBrandOrganicToggle(account) && !getBrandContentToggle(account))) {
        addValidationError('content_disclosure', account, $t('service.tiktok.content_disclosure_required'));
        return;
    }

    removeValidationError('content_disclosure', account);
}

const validatePrivacyLevel = (account) => {
    if (!getAccountPrivacyLevel(account)) {
        addValidationError('privacy_level', account, $t('service.tiktok.privacy_level_required'));
        return;
    }

    removeValidationError('privacy_level', account);
}
