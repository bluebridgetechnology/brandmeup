...
watch(() => cloneDeep(filter.value), throttle(() => {
    router.get(route('br.posts.index', {workspace: workspaceCtx.id}), pickBy(filter.value), {
        preserveState: true,
        only: ['posts', 'filter']
    });
}, 300))
...
const deletePosts = () => {
    router.delete(route('br.posts.deleteMultiple', {workspace: workspaceCtx.id}), {
        data: {
        ...
        <div class="w-full row-px">
            <Tabs> 
                <Tab @click="filter.status = null" :active="!$page.props.filter.status">{{ $t('general.all') }}</Tab>
                 <Tab @click="filter.status = 'pending'" :active="$page.props.filter.status === 'pending'">
                   Pending approval 
                </Tab>
