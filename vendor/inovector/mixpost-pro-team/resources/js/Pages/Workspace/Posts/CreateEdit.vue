...
const store = (data) => {
    router.post(route('br.posts.store', {workspace: workspaceCtx.id}), data, {
        onSuccess() {
...
const update = (data) => {
    ...
    axios.put(route('br.posts.update', {workspace: workspaceCtx.id, post: props.post.id}), data)
        .then(() => {
        ...
        if (mustRefreshPage) {
            router.visit(route('br.posts.edit', {workspace: workspaceCtx.id, post: props.post.id}));
        }
    }).finally(() => {
        triedToSave.value = true;
