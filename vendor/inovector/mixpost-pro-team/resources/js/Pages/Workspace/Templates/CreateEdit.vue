...
const store = (data, redirectToCreateRoute = false) => {
    ...
    axios.post(route('br.templates.api.store', {workspace: workspaceCtx.id}), data).then((response) => {
        notify('success', $t('template.template_created'));

        if (!redirectToCreateRoute) {
            router.get(route('br.templates.edit', {workspace: workspaceCtx.id, template: response.data.id}), {}, {
                preserveScroll: true,
            });
        ...
        router.get(route('br.templates.create', {workspace: workspaceCtx.id}));
    }).finally(() => {
        NProgress.done();
...
const update = (data) => {
    ...
    axios.put(route('br.templates.api.update', {workspace: workspaceCtx.id, template: props.template.id}), data)
        .then(() => {
    ...
    <div class="w-full mx-auto row-py">
        <PageHeader :title="title">
            <div class="flex">
                ...
                <Link v-if="template" :href="route('br.templates.create', { workspace: workspaceCtx.id })">
