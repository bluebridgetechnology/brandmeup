...
const useTemplate = () => {
    ...
    const createPost = (content) => {
        router.post(route('br.posts.store', {workspace: workspaceCtx.id}), {
            versions: [
    ...
    const deleteTemplate = (template) => {
        ...
        axios.delete(route('br.templates.api.delete', {
            workspace: workspaceCtx.id,
            template: template.id
        })).then(() => {
            router.get(route('br.templates.index', {workspace: workspaceCtx.id}), {}, {
                preserveScroll: true,
            });
        }).finally(() => {
            NProgress.done();
